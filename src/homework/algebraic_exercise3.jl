...
Similarly to the case of Taylor series, we discretize the function space of periodic functions via the two-sided sequence space
...
md"""
**1.** Prove that if $M \in B(\ell^1_\mathbb{Z})$, then $\| M \|_{B(\ell^1_\mathbb{Z})} = \max_{l \in \mathbb{Z}} \sum_{k \in \mathbb{Z}} |M_{k,l}|$.
"""

# ╔═╡ 4c9f56dd-b79d-4e7e-8b1d-38e91f6632df
md"""
**2.** Let $\omega > 0$.
If $u(t) = \sum_{k \in \mathbb{Z}} a_k e^{i k \omega t}$ and $v(t) = \sum_{k \in \mathbb{Z}} b_k e^{i k \omega t}$, then for their product we have $u(t) v(t) = \sum_{k \in \mathbb{Z}} (a*b)_k e^{i k \omega t}$ where
