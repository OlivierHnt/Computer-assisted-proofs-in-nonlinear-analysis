...
hint(md"Write $\left(M\bar{X}\right)^{-1}$ as $\left(I + M\bar{X}-I\right)^{-1}$, and mutliply to the left by $\bar{X}$ in order to get a power series expansion of $M^{-1}$")
...
hint(md"You may consider $F(X) = MX-I$ (or $F(X) = XM-I$).")
