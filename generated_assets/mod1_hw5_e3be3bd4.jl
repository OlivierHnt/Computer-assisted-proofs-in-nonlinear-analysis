...
md"""
Exercise C) can be used to enclose eigenvalues of a given matrix one by one. We now present an alternate strategy to enclose the entire spectrum at once (but not the corresponding eigenvectors), which can sometimes also be adapted in infinite dimension.
"""
...
$\begin{align}
\mathrm{Sp}(A) \subset \bigcup_{i=1}^d D\left(A_{i,i},\, \sum_{j\neq i} \vert A_{i,j}\vert\right),
\end{align}$
...
md"""
**1.** Using the Gershgorin circle theorem, get as tight as possible rigorous enclosures of all eigenvalues of $W_{7}$ (defined in exercise C)).
"""

# ╔═╡ 6488427e-9e34-46ef-a581-1768a8856234
hint(md"You may first compute numerically a matrix $P$ of approximate eigenvectors of $W_7$, then rigorously compute $\tilde W_{7} = P^{-1}W_7 P$, and finally apply the Gershgoring circle theorem to $\tilde W_{7}$.")

# ╔═╡ bc052916-db0e-43a2-bbb0-76b917d6f638
md"""
**2.** Try to prove that $W_{1001}$ has exactly one eigenvalue with positive real part.
"""

# ╔═╡ 3686e5b2-daac-470d-b9e3-4bd5706e5894
hint(md"You do not need to numerically diagonalize all of $W_{1001}$: for most rows, the corresponding Gershgorin disk already lies in the left half of the complex plane. ")
